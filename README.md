# End-to-End Sales Performance Dashboard (SQL Server + Power BI)
...
**Raw CSV Data → SQL Server (Staging) → Data Warehouse (Star Schema) → Analytics Views → Power BI Dashboards**

SQL Server is used for data quality validation, transformation, dimensional modeling, and analytics-ready views. Power BI is used to build interactive dashboards for executive reporting and decision support.
...
This user story outlines the specifications for building two dashboards using Power BI to help stakeholders, including **sales managers and executives**, analyze sales performance and customer behavior.
...
- Interactive charts for filtering and drill-down  
- Filters by:
...
## 📂 Repository Structure
    ...
    end-to-end-sales-performance-sql-power-bi/
    │
    ├── data/
    │ └── raw/
    │ ├── Customers.csv
...
4. **Analytics Views**
   ...
   - Monthly, weekly, product, and customer-level analytical views optimized for Power BI consumption  
...
The analytics layer is built on a dimensional star schema optimized for BI reporting and performance. The central fact table captures order-level transactions, while dimension tables provide business context for customers, products, location, and time. This model serves as the semantic layer consumed by the Power BI dashboards below.

![Sales Analytics Star Schema](powerbi/data_model.png)

---

### 📈 Sales Performance Dashboard
![Sales Dashboard](powerbi/dashboard_sales.png)

---

### 👥 Customer Analytics Dashboard
![Customer Dashboard](powerbi/dashboard_customers.png)
...
All datasets are ingested into SQL Server and transformed into a dimensional warehouse model before being consumed by Power BI.
